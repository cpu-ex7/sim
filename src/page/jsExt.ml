...
struct
  ...
  external appendChild : 'a -> unit = "" [@@bs.send.pipe: 'b]
...
struct
  ...
  external set : t -> string -> 'b -> unit = "" [@@bs.set_index]
  external get : t -> string -> 'b = "" [@@bs.get_index]
  let create () : t = Document.createElement "div"
  let append_child div elem = Element.appendChild div elem
  let set_style elem = set (get elem "style") "display" "none"
  let hide elem = set (get elem "style") "display" "none"
  let show elem = set (get elem "style") "display" "inline"
end
...
struct
  ...
  external set : t -> string -> 'b -> unit = "" [@@bs.set_index]
  external get : t -> string -> 'b = "" [@@bs.get_index]
  let create () : t = Document.createElement "table"
  let from_trs trs =
    ListLabels.fold_left ~f:(fun table tr -> table ||> Element.appendChild tr) ~init:(create ()) trs
end

module Tr =
struct
  type t
  let create () : t = Document.createElement "tr"
  let add_cell elem (tr : t) =
    tr |> Element.appendChild
      (Document.createElement "td" ||> Element.appendChild elem)
  let put_to_table (table : Table.t) (tr : t) = table |> Element.appendChild tr
end
