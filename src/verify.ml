...
let f parsed_program =
  ...
  for index = 0 to length - 1 do
    ...
    | OpSqrt, (FReg i, FReg j,     _)    -> !(verified).(index) <- (OpDivf, i, j, zero)
    | OpAbs,  (FReg i, FReg j,     _)    -> !(verified).(index) <- (OpMulf, i, j, zero)
    (* float変換命令 *)
    | OpMfc1,  (Reg i, FReg j,  _) -> !(verified).(index) <- (OpMfc1,  i, j, zero)
    | OpMfc2,  (Reg i, FReg j,  _) -> !(verified).(index) <- (OpMfc2,  i, j, zero)
