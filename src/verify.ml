...
let f parsed_program =
  ...
  for index = 0 to length - 1 do
    ...
    | OpJump, (Dest i, Empty, Empty) -> !(verified).(index) <- (OpJump, i, zero, zero)
    | OpJal,  (Dest i, Empty, Empty) -> !(verified).(index) <- (OpJal,  i, zero, zero)
    ...
    | OpSqrt, (Reg i, Reg j,     _)      -> !(verified).(index) <- (OpDivf, i, j, zero)
    | OpAbs,  (Reg i, Reg j,     _)      -> !(verified).(index) <- (OpMulf, i, j, zero)
    (* float変換命令 *)
    | OpMfc1,  (Reg i, Reg j,     _) -> !(verified).(index) <- (OpMfc1,  i, j, zero)
    | OpMfc2,  (Reg i, Reg j,     _) -> !(verified).(index) <- (OpMfc2,  i, j, zero)
    | OpRevn,  (Reg i, Reg j,     _) -> !(verified).(index) <- (OpRevn,  i, j, zero)
    | OpCvtsw, (Reg i, Reg j,     _) -> !(verified).(index) <- (OpCvtsw, i, j, zero)
    (* float比較命令 *)
    | OpEqf, (Reg i, Reg j, _)       -> !(verified).(index) <- (OpEqf, i, j, zero)
    | OpNef, (Reg i, Reg j, _)       -> !(verified).(index) <- (OpNef, i, j, zero)
    | OpLtf, (Reg i, Reg j, _)       -> !(verified).(index) <- (OpLtf, i, j, zero)
    | OpLef, (Reg i, Reg j, _)       -> !(verified).(index) <- (OpLef, i, j, zero)
    | OpGtf, (Reg i, Reg j, _)       -> !(verified).(index) <- (OpGtf, i, j, zero)
    | OpGef, (Reg i, Reg j, _)       -> !(verified).(index) <- (OpGef, i, j, zero)
    (* float制御命令 *)
    | OpBct, (Reg i, _, _)           -> !(verified).(index) <- (OpBct, i, zero, zero)
    | OpBcf, (Reg i, _, _)           -> !(verified).(index) <- (OpBcf, i, zero, zero)
    (* メモリ命令 *)
